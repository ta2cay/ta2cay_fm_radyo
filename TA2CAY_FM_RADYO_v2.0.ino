...
void saveCurrentFrequency() {
  ...
  tft.setTextSize(1);
  char successMsg[40];
  sprintf(successMsg, "%.1f %d. kanala kaydedildi", freqX10 / 10.0, slot + 1);
...
// Sinyal çubuklarını | simgesiyle göster
...
void drawMemoryScreen() {
  ...
  for (int i = 0; i < usedMemorySlots; i++) {
  ...
  // Boş slotları göster
  for (int i = usedMemorySlots; i < MEMORY_CHANNELS; i++) {
    tft.setCursor(5, 35 + i * 12);
    tft.setTextColor(ST7735_WHITE, ST7735_BLACK); // ST7735_GRAY yerine ST7735_WHITE
    char emptySlot[15];
  ...
  tft.setCursor(5, 145);
  tft.setTextColor(ST7735_GREEN, ST7735_BLACK);
  tft.print("Buton: Ana sayfaya don");
  
  tft.setCursor(5, 155);
  tft.setTextColor(ST7735_YELLOW, ST7735_BLACK);
  tft.print("Uzun basma: Kaydet");
}
...
void drawMainScreen() {
  ...
  tft.print("TA2CAY FM RADYO V2.0");
  ...
  // Sinyal çubukları - frekansın yanında
  drawSignalBars(freqX + freqWidth + 10, 25, getSignalStrength());
...
// Detay ekranı - düzeltilmiş ve temizlenmiş
void drawDetailScreen() {
  ...
  // Radyo bilgileri - sinyal | sembollerle
  tft.setCursor(5, 45);
  tft.setTextColor(ST7735_YELLOW, ST7735_BLACK);
  char radioStr[20];
  sprintf(radioStr, "RADYO: %.1f MHz ", freqX10/10.0);
  tft.print(radioStr);
  drawSignalBars(110, 45, getSignalStrength());
...
void loop() {
  ...
  if (rotaryEncoder.encoderChanged()) {
    ...
    // Normal frekans değişimi (sadece ana ekranda)
  ...
  if (rotaryEncoder.isEncoderButtonClicked()) {
    ...
    if (pressTime - lastButtonPress > 300) { // Debounce
      if (ignoreNextClick) {
        ...
        // Ekran döngüsü: Ana -> Detay -> Hafıza -> Ana
  ...
  // UZUN BASMA KONTROLÜ - 5 SANİYE (ANA EKRANDA)
  ...
  if (buttonCurrentlyPressed && !isButtonHeld && currentScreen == SCREEN_MAIN) {
    // Buton yeni basıldı ve ana ekrandayız
    buttonHoldStartTime = millis();
    isButtonHeld = true;
    longPressExecuted = false;
    Serial.println("Buton basili - sayim basladi");
  }
  
  if (buttonCurrentlyPressed && isButtonHeld && !longPressExecuted && currentScreen == SCREEN_MAIN) {
    // Buton hala basılı - süreyi kontrol et
    unsigned long holdTime = millis() - buttonHoldStartTime;
    
    if (holdTime >= 5000) {
      // 5 saniye tamamlandı!
      Serial.println("5 saniye tamamlandi - kaydediliyor!");
